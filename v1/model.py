...
class ConvNextModel_2fc(nn.Module):
    def __init__(self, num_classes):
        super().__init__()
        ...
        self.classifier_age = nn.Linear(in_features=768, out_features=1, bias=True)
        self.classifier_mask_gender = nn.Linear(in_features=768, out_features=6, bias=True)
    ...
    def forward(self, x):
        ...
        x=self.convnext(x)
        age=self.classifier_age(x)
        mask_gender=self.classifier_mask_gender(x)
        
        return age,mask_gender
...
# convnext_model = torchvision.models.convnext_small(weights='IMAGENET1K_V1')
