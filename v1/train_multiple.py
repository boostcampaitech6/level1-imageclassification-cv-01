...
def train(data_dir, model_dir, args):
    ...
    for epoch in range(args.epochs):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            age_value,mask_label,gender_label = torch.tensor(age_value).to(device),torch.tensor(mask_label).to(device),torch.tensor(gender_label).to(device)
            
            ###age###
            optimizer_age.zero_grad()

            logits = model_age(inputs)
            loss_age = criterion_age(logits, age_value)
            outs_age_list=[]
            for item in logits:
                if(item<30):
                    outs_age_list.append(0)
                elif(item<60):
                    outs_age_list.append(45)
                else:
                    outs_age_list.append(100)
            ...
            correct_predictions = (preds_age == age_value) & (preds_mask == mask_label) & (preds_gender == gender_label)
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                preds_mask,preds_gender =torch.argmax(logits_mask, dim=-1),torch.argmax(logits_gender, dim=-1)
                outs_age_list=[]
                for item in logits_age:
                    if(item<30):
                        outs_age_list.append(0)
                    elif(item<60):
                        outs_age_list.append(45)
                ...
                loss_item_age, loss_item_mask, loss_item_gender = criterion_age(logits_age, age_value).item(),criterion_mask(logits_mask, mask_label).item(),criterion_gender(logits_gender, gender_label).item()
